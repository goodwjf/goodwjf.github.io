...
KindEditor.plugin('table', function(K) { 
	...
	self.plugin.table = {
		...
		prop : function(isInsert) {
					...
					click : function(e) {
						...
						function modifyTable(){
							...
							if(border !== '' || borderColor !== ''){
 									var _rows = table.get(0).rows;
									for(var i = 0; i< _rows.length; i++){
											var _cells = _rows[i].cells;
											for(var j = 0; j<_cells.length;j++){ 
												_cells[j].style.border =  border+"px solid "+borderColor;
											}
									}
 							}
						...
						function insertNewTable(){
							...
							for (var i = 0; i < rows; i++) {
								...
								for (var j = 0; j < cols; j++) {
									//html += '<td>' + (K.IE ? '&nbsp;' : '<br />') + '</td>';
									//add 
									console.log('border:'+border+'px solid '+borderColor);
									html += '<td style="border:'+border+'px solid '+borderColor+'">' + (K.IE ? '&nbsp;' : '<br />') + '</td>';
			...
			spacingBox = K('[name="spacing"]', div).val(1),
	...
	self.clickToolbar(name, function(){self.plugin.table.prop(true); return;});
});
